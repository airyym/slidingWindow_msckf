...
StateVector::StateVector(std::shared_ptr<const Calibration> calibration)
...
const Quaternion& StateVector::getOrientationInGlobalFrame() const {
    assert(body_state_.get());
    return body_state_->getOrientationInGlobalFrame();
}

const Eigen::Vector3d& StateVector::getPositionInGlobalFrame() const {
    assert(body_state_.get());
    return body_state_->getPositionInGlobalFrame();
}

const Eigen::Vector3d& StateVector::getVelocityInGlobalFrame() const {
    assert(body_state_.get());
    return body_state_->getVelocityInGlobalFrame();
}

void StateVector::orientationCorrection(const Quaternion& orientation) {
    body_state_->orientationCorrection(orientation);
}

void StateVector::positionCorrection(const Eigen::Vector3d& position) {
    body_state_->positionCorrection(position);
}

void StateVector::velocityCorrection(const Eigen::Vector3d& velocity) {
    body_state_->velocityCorrection(velocity);
}

CameraPoseBuffer& StateVector::poses() {
    return poses_;
}

const CameraPoseBuffer& StateVector::poses() const {
    return poses_;
}

void StateVector::updateWithStateDelta(const Eigen::VectorXd& delta_x) {
