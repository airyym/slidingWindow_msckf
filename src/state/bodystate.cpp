...
: q_B_G_(Quaternion::identity()), rotation_to_this_frame_(Quaternion::identity()) {
    ...
    assert(!std::isnan(acceleration_estimate.norm()) && acceleration_estimate.norm() < 1e6);
...
Quaternion BodyState::propagateGyroscope(const BodyState &from_state, BodyState &to_state) {
    double dt = from_state.timeTo(to_state);
    ...
    if(omega > 0.00001)
    {
      ...
      Quaternion delta_quat(q_hat[0], q_hat[1], q_hat[2], k);
      perturb = delta_quat;
      perturb.normalize();
    }
    else
    {
      Eigen::Vector3d q_hat = 0.5*rotation_est_mid*dt;
      double k = 1;
      Quaternion delta_quat(q_hat[0], q_hat[1], q_hat[2], k);
      perturb = delta_quat;
      perturb.normalize();
